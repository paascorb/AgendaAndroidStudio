...
public class EditEventActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this, R.layout.spinner_item,colores);
    ...
    private void createAlertDialogs() {
        ...
        eventRepetitionRadioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            @Override
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                repeatTextView.setText("Repetir " + ((RadioButton) eventRepetitionDialogView.findViewById(checkedId)).getText().toString());
    ...
    private void setDuration(View view) {
        ...
        TimePickerDialog timePickerDialog = new TimePickerDialog(this, R.style.DurationPickerTheme, new TimePickerDialog.OnTimeSetListener() {
            ...
            public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
                setDurationButton.setText("DURACIÓN: " + Integer.toString(hourOfDay) + " HORAS " + Integer.toString(minute) + " MINUTOS");
            }
        }, 0, 0, true);
        timePickerDialog.setTitle("Duración");
                ...
                if (confirmInputs()) {
                    if (mEvent.isRecurring()) {
                        ...
                        new AlertDialog.Builder(this)
                                ...
                                .setMessage("¿Estás seguro que quieres editar este evento? Todas las apariciones del evento serán editadas.")
    ...
    private void getViewValues() {
        Date aDate = null;
        Spinner spinner = (Spinner) findViewById(R.id.color_edit);
        ...
        if (notColor == 0) {
            notColor = getResources().getInteger(R.color.rojo);
            mEvent.setColor(notColor);
        } if(notColor==1){
            notColor = getResources().getInteger(R.color.verde);
            mEvent.setColor(notColor);
        }
        if(notColor==2){
            notColor = getResources().getInteger(R.color.azul);
            mEvent.setColor(notColor);
        }
        if(notColor==3){
            notColor = getResources().getInteger(R.color.amarillo);
            mEvent.setColor(notColor);
        }
        if(notColor==4){
            notColor = getResources().getInteger(R.color.naranja);
            mEvent.setColor(notColor);
        }
        if(notColor==5){
            notColor = getResources().getInteger(R.color.morado);
            mEvent.setColor(notColor);
        }
        if(notColor==6){
            notColor = getResources().getInteger(R.color.marron);
            mEvent.setColor(notColor);
        }
